...
3. **读取 AGENTS.md 当前目录结构**：获取模板基线，作为结构提案的基础。
...
- 每个文件标注来源（"← 现有文件" 或 "待创建"）
- 结构必须符合 AGENTS.md 中的依赖层级（types → config → repo → service → runtime → ui）
- 参照 AGENTS.md 中的目录结构模板，但根据实际项目内容填充
...
### Phase 2: 更新 AGENTS.md 目录结构

目标：将确认后的目录结构写入 AGENTS.md 的「仓库目录结构」段。

**步骤**：

1. 用户确认结构后，重新读取 `AGENTS.md`（确保拿到最新版本）。
...
- 保持与 AGENTS.md 整体 Markdown 风格一致（中文注释、简洁列表）
...
| AGENTS.md                       | 检查文件存在且包含关键段落                                                                    | **阻塞**                   |
| Hooks 配置                      | 检查 `.claude/settings.json` 或 `.claude/hooks` 是否存在（Claude Code 专用，Kimi 用户可跳过） | 建议                       |
...
- **目录结构精简** — AGENTS.md 中的目录树只列架构骨架和关键文件（~25 行），不逐文件展开标注。Claude 可通过 Glob/Grep 自行发现具体文件。
- **不破坏已有配置** — 更新 AGENTS.md 时只改「仓库目录结构」段，与 claudemd-updater 的原则写入互不干扰。
...
- **一次启动** — 本 skill 对每个项目只执行一次。重复执行时先提醒用户检查 AGENTS.md 是否已有当前项目的目录结构。
